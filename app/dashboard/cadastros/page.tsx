    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-1 px-4 pt-4 border-b border-gray-100">
          {TABS.map(t => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 text-sm font-semibold rounded-t-lg border-b-2 -mb-px transition-all ${
                tab === t.id
      ...
      <Dialog open={dialogCliente} onClose={() => setDialogCliente(false)}
        ...
        <Field label="Nome do Cliente *">
          <input className="input-light" value={formCliente.nome_cliente} onChange={e => setFormCliente(p => ({...p, nome_cliente:e.target.value}))} />
        </Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label="CNPJ">
            <input className="input-light" value={formCliente.cnpj??''} onChange={e => setFormCliente(p => ({...p, cnpj:e.target.value}))} />
          </Field>
          <Field label="Telefone">
            <input className="input-light" value={formCliente.telefone??''} onChange={e => setFormCliente(p => ({...p, telefone:e.target.value}))} />
          </Field>
        </div>
        <Field label="Contato">
          <input className="input-light" value={formCliente.contato??''} onChange={e => setFormCliente(p => ({...p, contato:e.target.value}))} />
        </Field>
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Status">
            <select className="input-light" value={formCliente.status} onChange={e => setFormCliente(p => ({...p, status:e.target.value}))}>
        ...
        <div className="flex gap-2 pt-2">
          <button onClick={() => setDialogCliente(false)} className="btn-outline flex-1">Cancelar</button>
          <button onClick={saveCliente} disabled={!formCliente.nome_cliente || saving} className="btn-gradient flex-1">
      ...
      <Dialog open={dialogVigilante} onClose={() => setDialogVigilante(false)}
        ...
        <Field label="Nome Completo *">
          <input className="input-light" value={formVigilante.nome_completo} onChange={e => setFormVigilante(p => ({...p, nome_completo:e.target.value}))} />
        </Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label="CPF *">
            <input className="input-light font-mono" value={formVigilante.cpf} onChange={e => setFormVigilante(p => ({...p, cpf:e.target.value}))} />
          </Field>
          <Field label="Função">
            <select className="input-light" value={formVigilante.funcao_id??''} onChange={e => setFormVigilante(p => ({...p, funcao_id:e.target.value}))}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="CNV">
            <input className="input-light font-mono" value={formVigilante.cnv??''} onChange={e => setFormVigilante(p => ({...p, cnv:e.target.value}))} />
          </Field>
          <Field label="Extensão EEA">
            <input className="input-light font-mono" value={formVigilante.extensao_escolta_armada??''} onChange={e => setFormVigilante(p => ({...p, extensao_escolta_armada:e.target.value}))} />
          </Field>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <Field label="Valor Padrão (R$)">
            <input type="number" className="input-light" value={formVigilante.valor_padrao_pago??0} onChange={e => setFormVigilante(p => ({...p, valor_padrao_pago:Number(e.target.value)}))} />
          </Field>
          <Field label="Status">
            <select className="input-light" value={formVigilante.status} onChange={e => setFormVigilante(p => ({...p, status:e.target.value}))}>
              <option value="ativo">Ativo</option>
              <option value="inativo">Inativo</option>
            </select>
          </Field>
        ...
        <div className="flex gap-2 pt-2">
          <button onClick={() => setDialogVigilante(false)} className="btn-outline flex-1">Cancelar</button>
          <button onClick={saveVigilante} disabled={!formVigilante.nome_completo || !formVigilante.cpf || saving} className="btn-gradient flex-1">
            {saving ? 'Salvando...' : 'Salvar'}
          </button>
        </div>
      </Dialog>
      ...
      <Dialog open={dialogVeiculo} onClose={() => setDialogVeiculo(false)}
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Placa *">
            <input className="input-light font-mono uppercase" value={formVeiculo.placa} onChange={e => setFormVeiculo(p => ({...p, placa:e.target.value.toUpperCase()}))} />
          </Field>
          <Field label="Tipo">
            <select className="input-light" value={formVeiculo.tipo_id??''} onChange={e => setFormVeiculo(p => ({...p, tipo_id:e.target.value}))}>
              <option value="">Selecione...</option>
        ...
        <Field label="Modelo">
          <input className="input-light" value={formVeiculo.modelo??''} onChange={e => setFormVeiculo(p => ({...p, modelo:e.target.value}))} />
        </Field>
        <Field label="Status">
          <select className="input-light" value={formVeiculo.status} onChange={e => setFormVeiculo(p => ({...p, status:e.target.value}))}>
            <option value="ativo">Ativo</option>
            <option value="inativo">Inativo</option>
        ...
        <div className="flex gap-2 pt-2">
          <button onClick={() => setDialogVeiculo(false)} className="btn-outline flex-1">Cancelar</button>
          <button onClick={saveVeiculo} disabled={!formVeiculo.placa || saving} className="btn-gradient flex-1">
            {saving ? 'Salvando...' : 'Salvar'}
          </button>
        </div>
      </Dialog>
